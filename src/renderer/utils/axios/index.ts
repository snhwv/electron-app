...
// const { createMessage } = usedMessage();
...
const transform: AxiosTransform = {
  ...
  transformRequestHook: (res: any, options: RequestOptions) => {
    ...
    // const needLogin =
    //   data && Reflect.has(data, 'status') && status === ResultEnum.NEEDAUTH;
    // if (needLogin) {
    //   Persistent.removeSession('TOKEN__');
    //   window.location.hash = '/login';
    //   return;
    // }

    // const multipleMsg =
    //   data && Reflect.has(data, 'status') && status === ResultEnum.MULTIPLEMSG;

    // let extralMsg = '';
    // if (multipleMsg) {
    //   if (is(res.data, 'Array')) {
    //     extralMsg = result.map((d: any) => d.field + d.msg).join('-');
    //   }
    // }
    // const warningMsg = ` (${status}):
    // ${extralMsg ? `${message}（${extralMsg}）` : message}`;
  ...
  requestInterceptors: (config, options) => {
    // 请求之前处理config
    const token = Persistent.getSession('TOKEN__');
  ...
  responseInterceptorsCatch: (error: any) => {
    ...
    // const { response, status, message, config } = error || {};
    ...
    // try {
    //   if (status === "ECONNABORTED" && message.indexOf("timeout") !== -1) {
